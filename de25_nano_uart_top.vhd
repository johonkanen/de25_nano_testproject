...
architecture rtl of de25_nano_uart_top is
    ...
    -- HPS's own h2f_reset output - per the TRM, this should be looped back
    ...
    -- itself is actually ready, rather than an arbitrary system_reset
    -- power-on delay that has no real relationship to HPS readiness.
    --
    -- (The actual root cause of this project's LWH2F bring-up hang turned
    -- out to be unrelated to this wiring entirely - see
    -- memory/de25_nano_lwh2f_hps_first_regression.md - it was the QSF
    -- HPS_INITIALIZATION/QSPI_OWNERSHIP boot-order assignments. This
    -- synchronizer is being wired back in on its own merits, as a more
    -- correct design than gating the bridge/HPS interface on an unrelated
    -- fabric-only wait loop.)
    --
    ...
    -- into synchronous logic risks a metastable release edge, so it's
    -- brought through a standard async-assert/sync-release 3-FF chain,
