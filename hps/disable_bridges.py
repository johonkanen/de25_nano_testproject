...
agilex_hps.ip is vendored verbatim from the DE25-Nano GHRD
...
standalone Linux-capable host with no fabric AXI ports:
...
Same pattern as de25_std_testproject/hps/disable_bridges.py - see that
project for the rationale (a standalone HPS keeps de25_nano_uart_top's own
fabric register block untouched, no fabric<->HPS memory-mapped access).
