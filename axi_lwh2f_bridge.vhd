...
-- de25_nano_uart_top.vhd is reachable from HPS software over LWH2F,
-- exactly like it already is over the fabric UART.
--
-- Ported verbatim from de25_std_testproject/axi_lwh2f_bridge.vhd (same
-- author, same board family) - itself ported from
-- ~/dev/datacenter_peak_shaving/fpga/agilex/de25/axi_led.vhd. The state
-- machine and the generic-package-parameter pattern (so this entity
-- compiles against any fpga_interconnect_pkg instance, not just a
-- hardcoded one) are unchanged; the demo LED/local register bits are
-- dropped. Nothing board-specific in this file - see de25_std_testproject's
-- own linux/README.md and hps/baremetal_lwh2f_regs/README.md for the real
-- story of what it took to get LWH2F working end to end there (a
-- reset-polarity bug in the fabric RTL, and a missing Ncore CCU crossbar
-- routing window ATF normally programs) - worth checking for here too.
--
